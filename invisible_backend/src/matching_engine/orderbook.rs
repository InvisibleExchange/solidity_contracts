...
use super::domain::{Order, OrderSide, OrderType, OrderWrapper, SharedOrderInner};
...
impl OrderBook {
    ...
    pub fn process_order(&mut self, order: OrderRequest) -> OrderProcessingResult {
        ...
        match order {
            OrderRequest::NewLimitOrder {
                ...
                let inner_shared = Arc::new(Mutex::new(SharedOrderInner {
                    order,
                    order_id,
                    order_side: side,
                    signature,
                ...
                for (price, qty) in prices.iter().zip(qtys.iter()) {
                    let quote_qty =
                        get_quote_qty(*qty, *price, self.order_asset, self.price_asset, None);

                    let wrapper = OrderWrapper {
                        order: inner_shared.clone(),
                        price: *price,
                        qty_left: *qty,
                    };
    ...
    pub fn retry_order(
        ...
        match order {
            OrderRequest::NewLimitOrder {
                ...
                // ? remove orders that failed to match to prevent endless loops
                if let Some(failed_order_ids) = failed_order_ids {
                    ...
                    pending_orders = opposite_queue.pop_with_failed_ids(&failed_order_ids);
                }

                let quote_qty =
                    get_quote_qty(qty, prices[0], self.order_asset, self.price_asset, None);

                let queue = match side {
                    OrderSide::Bid => &mut self.bid_queue,
                    OrderSide::Ask => &mut self.ask_queue,
                };
                let inner_order = queue.get_pending_inner_order(order_id);
                if inner_order.is_none() {
                    return vec![Err(Failed::OrderNotFound(order_id))];
                }
                let wrapper = OrderWrapper {
                    order: inner_order.unwrap(),
                    price: prices[0],
                    qty_left: qty,
                };
                ...
                // ? reinsert orders that failed to match
                let opposite_queue = match side {
                    OrderSide::Bid => &mut self.ask_queue,
                    OrderSide::Ask => &mut self.bid_queue,
                };
                for (order, ts) in pending_orders {
                    ...
                    opposite_queue.insert(ord_id, order.price, ts, order.clone());
                }
            }
            _ => {}
        }
    ...
    pub fn process_order_internal(
        ...
        if let Some(opposite_order) = opposite_order_result {
            let mut order_inner = opposite_order.order.lock();

            // ? Check that the order being matched has not expired
            if order_inner.order.has_expired() {
                let order_queue = match order_inner.order_side {
                    OrderSide::Bid => &mut self.bid_queue,
                    OrderSide::Ask => &mut self.ask_queue,
                };

                order_queue.cancel(order_inner.order_id, 0, true);
            ...
            if could_be_matched {
                ...
                let is_spot: bool;
                if let Order::Spot(_) = order_inner.order {
                    is_spot = true;
                } else {
                    is_spot = false;
                }

                match &mut order_inner.order {
                    Order::Spot(ord) => {
                        ord.set_hash();
                    }
                    Order::Perp(ord) => {
    ...
    fn process_order_cancel(
        ...
        order_queue.cancel(order_id, user_id, false);
    ...
    fn process_order_amend(
        ...
        let order_wrappers: Vec<OrderWrapper>;
        let mut prev_price: f64 = 0.0;
        if match_only {
            if let Some(mut wrapper) = order_queue.get_order(order_id) {
                prev_price = wrapper.price;

                wrapper.price = new_prices[0];

                order_wrappers = vec![wrapper.clone()];
            } else {
                results.push(Err(Failed::OrderNotFound(order_id)));
                return;
            }
        } else {
            if let Some(wrappers) = order_queue.amend(
                order_id,
                user_id,
                &new_prices,
                new_expiration,
        ...
        for (i, mut wrapper) in order_wrappers.into_iter().enumerate() {
            ...
            let qty_left = self.process_order_internal(
                results,
                order_id,
                self.order_asset,
                self.price_asset,
            ...
            if qty_left == 0 && !match_only {
                let order_queue = match side {
                    OrderSide::Bid => &mut self.bid_queue,
                    OrderSide::Ask => &mut self.ask_queue,
                };

                order_queue.remove_order(order_id, user_id, true);
            }
            ...
            if qty_left > 0 && match_only {
                ...
                wrapper.price = prev_price;

                order_queue.replace_order(order_id, wrapper)
            }
        }
    }
    ...
    fn order_matching(
        ...
        let opposite_order = opposite_wrapper.order.lock();
        let current_order = order_wrapper.order.lock();
        ...
        if qty < opposite_wrapper.qty_left {
            ...
            opposite_queue.modify_current_order(order_id, opposite_wrapper.qty_left - qty);

            // ? Store pending orders
            // opposite_queue.store_pending_order(opposite_wrapper.clone());
            same_queue.store_pending_order(order_wrapper.clone());
    ...
    fn order_matching_market_bid(
        ...
        if opposite_quote_qty < quote_qty {
            ...
            // ? Store pending orders
            opposite_queue.store_pending_order(opposite_wrapper.clone());
            // opposite_queue.store_pending_order(order_wrapper.clone());
    ...
    pub fn _restore_spot_inner(
        ...
        if let Ok(limit_order) = LimitOrder::try_from(order.order.unwrap()) {
            ...
            let inner_order = Arc::new(Mutex::new(SharedOrderInner {
                order_id,
                order: Order::Spot(limit_order.clone()),
                order_side,
                signature,
                user_id,
            }));
    ...
    pub fn _restore_perp_inner(
        ...
        if let Ok(perp_order) = PerpOrder::try_from(order.order.unwrap()) {
            ...
            let inner_order = Arc::new(Mutex::new(SharedOrderInner {
                order_id,
                order: Order::Perp(perp_order.clone()),
                order_side,
                signature,
                user_id,
            }));
    ...
    pub fn get_market_price(&mut self) -> Result<u64, String> {
        ...
        let bid_price_ = top_bid_order.unwrap().price;

        let ask_price_ = top_ask_order.unwrap().price;
